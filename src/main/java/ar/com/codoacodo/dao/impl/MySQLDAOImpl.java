...
public class MySQLDAOImpl implements DAO {
    ...
    public void update(Articulo articulo) throws Exception  {
            String sql = "UPDATE " + this.tableName +
                    " SET titulo = ?, precio = ?, autor = ?, imagen = ?" +
                    " WHERE id = ?";

                    sql="UPDATE `23049-db`.`articulos` SET `titulo`='prueba2' WHERE  `id`=11;";
            ...
            PreparedStatement pst = con.prepareStatement(sql);
            /*
            // Asignar los valores usando los métodos setX de PreparedStatement
    ...
    private ArrayList<Articulo> findBySQL(String sql) throws Exception {
        ...
        while (res.next()) {
            ...
            var articulo = new Articulo(_id, titulo, imagen, autor, precio, Boolean.parseBoolean(novedad), codigo,
                    LocalDateTime.now());
